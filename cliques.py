...
def compute_cliques(graphs, config, logger):
    logger.info('Start computing cliques')
    cliques = {}
    cliques['max_cliques_train'] = detect_max_cliques(graphs['G_train'], 'train', config, logger)
    # cliques['support_cliques_train'], _ = find_support(cliques['max_cliques_train'])
    # cliques['max_cliques_test'] = detect_max_cliques(graphs['G_test'], 'test', config, logger)
    ...
    cliques['sampler'] = CliqueSampler(cliques['max_cliques_train'], graphs['simplicies_train'], logger, config)
...
class CliqueSampler:
    def __init__(self, max_cliques, hyperedges, logger, config):
        self.config = config
        ...
        self.r, self.seq, self.n_collected = self.optimize(config['beta'], self.N, self.rho)
